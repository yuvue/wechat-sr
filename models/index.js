...
const getContacts = async function(user_id) {
    // 找到所有联系人
    let c1 = (await Contact.aggregate([{ $match: { user_id } }])).map(item => ({
        ...item,
        addFrom: user_id
    }));
    ...
    ])).map(item => ({ ...item, addFrom: item.contact_id }));
    let contacts = [...c1, ...c2];
    // 加入消息列表、备注等到每个联系人
...
const saveMessage = async function(data) {
    ...
    let remark = (await ContactInfo.findOne({ contact_id: data.from_id }))
        .remark;
